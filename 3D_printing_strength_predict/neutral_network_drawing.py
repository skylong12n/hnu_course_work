...
# 创建绘图
fig, ax = plt.subplots(figsize=(14, 9)) # 稍微增大画布尺寸
ax.set_aspect('equal')
...
for i, y in enumerate(input_neurons_y):
    if i == 3: # 虚线部分
        ax.text(input_layer_x, y - neuron_spacing/2 + 0.1, "...", ha='center', va='center', fontsize=10, color='black') # 字体小一点
        continue
...
for i, y in enumerate(hidden1_neurons_y):
    if i == 3:
        ax.text(hidden1_layer_x, y - neuron_spacing/2 + 0.1, "...", ha='center', va='center', fontsize=10, color='black') # 字体小一点
        continue
...
for i, y in enumerate(hidden2_neurons_y):
    if i == 3:
        ax.text(hidden2_layer_x, y - neuron_spacing/2 + 0.1, "...", ha='center', va='center', fontsize=10, color='black') # 字体小一点
        continue
...
# --- 输出层 (1个神经元) ---
output_neuron_y = (max(input_neurons_y) + min(input_neurons_y)) / 2  # 大概居中
